    ...
    private Operation parseMethod(Method method) {
        ...
        if (responseClass != null
            ...
            if (isPrimitive(responseClass)) {
                ...
                if (property != null) {
                    Property responseProperty = RESPONSE_CONTAINER_CONVERTER.withResponseContainer(responseContainer, property,responseClass);
                    operation.response(apiOperation.code(), new Response()
                            .description("successful operation")
                            .schema(responseProperty)
                            .headers(defaultResponseHeaders));
                    if("ResponseDto".equalsIgnoreCase(responseContainer)){
                        Map<String, Model> modelMap = ModelConverters.getInstance().read(method.getGenericReturnType());
                        Model responseDtoModel = modelMap.get("ResponseDto");
                        Map<String, Property>  propertyMap= responseDtoModel.getProperties();
                        Map<String, Property>  propertyMapNew=new HashMap<>();
                        for(String keyx :propertyMap.keySet()){
                            if(keyx.equals("data")){
                                property.setDescription("结果信息");
                                propertyMapNew.put(keyx,property );
                            }else {
                                Property propertyx =  propertyMap.get(keyx);
                                if(keyx.equals("code")){
                                    propertyx.setDescription("错误码");
                                }else {
                                    propertyx.setDescription("错误信息");
                                }
                                propertyMapNew.put(keyx,propertyx);
                            }
                        }
                        responseDtoModel.setProperties(propertyMapNew);
                        if (responseClass.getTypeName().equals("java.util.Map")) {
                            swagger.model("ResponseDto<map>",responseDtoModel);
                        }else if (responseClass.getTypeName().equals("java.util.List")){
                            swagger.model("ResponseDto<list>",responseDtoModel);
                        }else {
                            swagger.model("ResponseDto<"+property.getType()+">",responseDtoModel);
                        }
                    ...
                    if("ResponseDto".equalsIgnoreCase(responseContainer)){
                        ...
                        for(String keyx :propertyMap.keySet()){
                            if(keyx.equals("data")){
                                ...
                                Property property =  propertyMap.get(keyx);
                                if(keyx.equals("code")){
                                    property.setDescription("错误码");
                                }else {
                                    property.setDescription("错误信息");
                                }
                                propertyMapNew.put(keyx,property);
                            }
                        }
                        responseDtoModel.setProperties(propertyMapNew);
                        swagger.model("ResponseDto<"+key+">",responseDtoModel);
            }
        }
...
//        this.readImplicitParametersAll(method, operation);
